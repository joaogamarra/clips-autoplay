...
const Player: React.FC = () => {
	const [{ clips, currentClip, clipIndex, searchMode }, dispatch] = useStateValue()
	...
	const loadMoreClips = useCallback(async () => {
		...
		if (searchMode === searchType.category) {
			queryId = clips.data[0].game_id
		}

		const newClips = await getClips(queryId, searchMode, after)

		dispatch(updateClips(newClips))
	}, [clips, searchMode, dispatch])
